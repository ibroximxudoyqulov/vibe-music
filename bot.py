...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"<h1>VibeStudio Master Bot is 100% Active!</h1>")
...
SECRET_CHANNEL_ID = -1004428420836  # Maxfiy Arxiv kanali
...
def get_user(user_id):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, username, lang, tier, tier_expires, joined_date FROM users WHERE user_id=?", (user_id,))
    row = cursor.fetchone()
    conn.close()
    return row
...
# ==================== 4. LUG'AT (3 TA TILDA SOF VA XATOSIZ) ====================
TEXTS = {
    "uz": {
...
def message_handler(message):
    ...
    # HAVOLA KELGANDA TOZALAB AVTOMATIK YUKLASH
